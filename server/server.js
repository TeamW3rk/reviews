...
const runServer = async () => {
  ...
  Client = await MongoClient.connect(`mongodb://localhost/${DB_NAME}`);
  mdb = Client.db(DB_NAME);
  // const restaurants  = await mdb.collection('Restaurant').find().limit(2).toArray();
  ...
  router.get('/summary', amw(async (req, res, next) => {
    ...
    // mdb = Client.db(DB_NAME);
    const restaurants  = await mdb.collection('Restaurant').find().limit(2).toArray();
    // const restaurants  = await Models.restaurantModel.find().limit(4).exec();
    const restaurant   = chance.pickone(restaurants);
    console.log(restaurant);
    const totalReviews = await mdb.collection('Reviews').find({ restaurant: restaurant._id }).count();
